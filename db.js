...
async function initPostgres() {
  ...
  await createPgTables(pool);

  return buildPgWrapper(pool);
}
...
function buildPgWrapper(pool) {
  function toSQL(sql, params) {
    ...
    const pgSql = sql
      ...
      .replace(/datetime\('now', '\+(\d+) (\w+)'\)/gi, "NOW() + INTERVAL '$1 $2'")
  ...
  return {
    ...
    run: async (sql, params) => {
      ...
      try {
        ...
        return { lastID: r.rows[0] ? r.rows[0].id : null, changes: r.rowCount || 0 };
      } catch (e) {
        const r = await pool.query(pgSql, pgParams);
